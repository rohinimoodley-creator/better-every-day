                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
                  <div>
                    ...
                    <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', margin: 0 }}>
                      Occasional 10–60 second adjustments to encourage regular position changes.
                    </p>
                  </div>
                  ...
                  <label className="toggle-switch">
                    <input
                      type="checkbox"
                      checked={microMovementSettings.postureResetsEnabled}
                      onChange={e => togglePostureResets(e.target.checked)}
                    />
                    <span className="toggle-slider" />
                  </label>
                </div>

                {microMovementSettings.postureResetsEnabled && (
                  <div>
                    {/* Active Posture Card */}
                    {(() => {
                      const currentPosture = POSTURE_RESETS_DATA[activePostureIndex] || POSTURE_RESETS_DATA[0];
                      return (
                        <div style={{ background: 'var(--bg-tertiary)', padding: '0.9rem 1.1rem', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-subtle)', marginBottom: '0.75rem' }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.35rem' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.45rem' }}>
                              <span style={{ fontSize: '1.3rem' }}>{currentPosture.icon}</span>
                              <strong style={{ fontSize: '0.92rem', color: 'var(--text-primary)' }}>
                                {currentPosture.title}
                              </strong>
                              <span className="pill-badge primary" style={{ fontSize: '0.64rem' }}>
                                {currentPosture.duration}
                              </span>
                            </div>

                            <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>
                              {activePostureIndex + 1} of {POSTURE_RESETS_DATA.length}
                            </span>
                          </div>

                          <p style={{ fontSize: '0.82rem', color: 'var(--text-secondary)', margin: '0 0 0.4rem 0', lineHeight: 1.45 }}>
                            {currentPosture.instruction}
                          </p>

                          <div style={{ fontSize: '0.74rem', color: 'var(--accent-primary)', fontStyle: 'italic', marginBottom: '0.85rem' }}>
                            💡 {currentPosture.tip}
                          </div>
                          ...
                          <div style={{ display: 'flex', gap: '0.5rem' }}>
                            <button
                              type="button"
                              onClick={() => handleCompletePosture(currentPosture)}
                              className="btn btn-primary btn-sm"
                              style={{ flex: 1, fontSize: '0.78rem', justifyContent: 'center' }}
                            >
                              <Check size={13} /> Complete Reset
                            </button>

                            <button
                              type="button"
                              onClick={() => handleSkipPosture(currentPosture)}
                      ...
                      {POSTURE_RESETS_DATA.map((p, idx) => (
                        <button
                          ...
                          style={{
                            ...
                            borderRadius: 'var(--radius-pill)',
                            border: idx === activePostureIndex ? '1.5px solid var(--accent-primary)' : '1px solid var(--border-subtle)',
                            background: idx === activePostureIndex ? 'var(--accent-primary-light)' : 'var(--bg-tertiary)',
                            color: idx === activePostureIndex ? 'var(--accent-primary)' : 'var(--text-muted)',
                            fontSize: '0.72rem',
                            fontWeight: 600,
                            cursor: 'pointer'
                          }}
